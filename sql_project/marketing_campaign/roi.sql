WITH t1 AS (SELECT 'Campaign № 1' AS ads_campaign,
                   100*((SELECT SUM(price)
                         FROM (SELECT unnest(product_ids) AS product_id 
                                FROM orders 
                                WHERE order_id IN (SELECT order_id FROM user_actions WHERE
                                ...
                                AND order_id NOT IN (SELECT order_id 
                                                    FROM user_actions WHERE action = 'cancel_order'))) t3 
                        JOIN products USING(product_id))-250000)::DECIMAL/250000 AS roi),

t2 AS (SELECT 'Campaign № 2' AS ads_campaign,
               100*((SELECT SUM(price)
                     FROM (SELECT unnest(product_ids) AS product_id 
                     FROM orders WHERE order_id IN (SELECT order_id FROM user_actions WHERE 
                                 ...
                                 AND order_id NOT IN (SELECT order_id 
                                                        FROM user_actions WHERE action = 'cancel_order'))) t4 
                    JOIN products USING(product_id))-250000)::DECIMAL/250000 AS roi)
